...
test("refreshes pull requests every minute even when the modal is closed", async () => {
  vi.useFakeTimers();
  const listGitHubPullRequests = vi.fn(async () => createDashboardPullRequests());
  ...
  expect(listGitHubPullRequests).toHaveBeenCalledTimes(2);
});

test("keeps refreshing pull requests every minute while the modal is open", async () => {
  vi.useFakeTimers();
  const listGitHubPullRequests = vi.fn(async () => createDashboardPullRequests());
  const tree = render(
    <DiffdiffApp
  ...
  expect(listGitHubPullRequests).toHaveBeenCalledTimes(2);
  expect(getAppText(tree)).toContain("GitHub PRs");
  expect(getAppText(tree)).toContain("/ ship");
});
